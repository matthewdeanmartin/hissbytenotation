...
behind hissbytenotation's tkinter GUI so that contributors can extend or modify it
confidently.
...
```bash
uv run hissbytenotation gui        # subcommand
uv run hissbytenotation --gui      # top-level flag
uv run hissbytenotation-gui        # gui-scripts entry point (pyproject.toml)
```

All three converge on `hissbytenotation.gui.app:launch_gui`.
...
def _fetch(scope_str):
    ...
    from hissbytenotation.services import read_current_report
...
1. Add a sidebar entry in `hissbytenotationApp._build_ui`:
...
```python
with patch("hissbytenotation.services.read_current_report",
